...
@Composable
fun TextFieldForm(value: String, placeholder: String, input: (String) -> Unit) {
    var borderColor = colors.outline
    if(value.trim().isNotEmpty()) borderColor = colors.primary
    OutlinedTextField(
        value = value,
        onValueChange = { input(it) },
        textStyle = MethodistTheme.typography.textInFiled.color(colors.title),
        modifier = Modifier
            .fillMaxWidth()
            .shadow(
        ...
        placeholder = {
            Text(
                text = placeholder,
                style = MethodistTheme.typography.textInFiled.color(colors.hint)
            )
        },
        singleLine = true,
        maxLines = 1,
